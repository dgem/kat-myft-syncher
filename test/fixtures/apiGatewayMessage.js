...
function message(msg) {
	...
	return {
	  ...
	  "body": {
	    "topic": msg.topic || "membership_users_v1",
...
function bodyMessages(messages) {
	return messages.map((ftMsg)=>{
		ftMsg = Object.apply({}, ftMsg);
		ftMsg.messageType = ftMsg.messageType;
	});
}
...
function ftMessage(ftMsg){
	...
	return {
		...
		"customHeaders": {
			"FT-Transaction-Id": msg.ftTransactionId || uuid()
		},
...
function ftMessageBody(msgType, msgBody) {
	msgBody = Object.apply({}, msgBody);
	let body;
	switch (msgType) {
		case 'UserCreated':
			let user = Object.apply({}, msgBody.user);
			body = {
				user:{
					id: user.userId || uuid(),
					email: user.email,
		...
		case 'LicenceSeatAllocated':
			let licenceSeatAllocated = Object.apply({}, msgBody.licenceSeatAllocated);
			body = {

...
function syncMyFTUser(userId, licenceId){
	...
	.then(usersResp=>{
		// log.silly({operation:'usersResp', groupResp: JSON.stringify(usersResp)});
...
function syncUserOnLicence(userId, licenceId){
	...
	then(usersResp=>{
		...
		if (userIds && userIds.indexOf(userId)>0){
			...
			// return {user:{uuid, status:'synchronisationIgnored', reason:'alreadyInAllUsersGroup', licence:licence.id}};
...
function syncUserInGroups(userId, groupIds){
	...
	return Promise.all(groupSyncs).then(syncs=>{
		...
		// return {user:{uuid:userId, status:'synchronisationComplete', groupIds, syncs:JSON.stringify(syncs)}};
		return syncs;
	});
}

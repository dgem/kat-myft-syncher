...
function process(event) {
	if (event.body) {
		let body = event.body instanceof(String)?JSON.parse(event.body):event.body;
		if (body.messages && body.messages instanceof Array) {
			let results = body.messages.map((message)=>{
	...
	} else if (event.resource === '/gtg' || event.resource === '/__gtg') {
		return status.ping(event);
	// TODO
...
function userCreated(message) {
	let body = message.body instanceof(String)?JSON.parse(message.body):message.body;

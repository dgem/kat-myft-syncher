...
function formatResult(result) {
	log.silly({operation: 'formatResult', result:JSON.stringify(result)});
	let statusCode=200;
	...
	if (result instanceof(errors.Fatal)){
		return {statusCode: 500, body:`${result.name}: ${result.message}`};
	} else if (result instanceof(errors.Warning)){
		return {statusCode: 200, body:`${result.name}: ${result.message}`};
	}

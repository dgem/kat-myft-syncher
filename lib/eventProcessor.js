...
function processEvent(event) {
	return new Promise((resolve, reject) => {
		...
		} else if (event.httpMethod && event.resourceId && event.resourcePath) {
			log.silly({operation:'processEvent', status:'API GATEWAY PROXY INTEGRATION'});
			return processAPIGWProxy(event);
		} else {
			dump(event);
			log.error({operation:'processEvent', status:'Unknown Event'});
		}
	})
	.catch(error => {
		dump(record);

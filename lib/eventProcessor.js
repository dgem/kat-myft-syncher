	...
	return new Promise((resolve, reject) => {
		...
		try {
			if (event.Records) {
				...
				log.error({operation:'processEvent', status:msg, event:stringify(eventEssentials(event))});
				return errors.Warning(msg);
			}
		} catch (error) {
			log.error({operation:'processEvent', error, event:stringify(eventEssentials(event))});
			return error;
		};
		resolve(result);
	})
	.then(result=>{
		log.debug({operation:'processEvent', result:stringify(result), event:stringify(eventEssentials(event))});

	...
	return new Promise((resolve, reject) => {
		...
		try {
			if (event.Records) {
				...
				log.error({operation:'processEvent', status:msg, event:logHelper.stringify(eventEssentials(event))});
				return errors.Warning(msg);
			}
		} catch (error) {
			log.error({operation:'processEvent', error, event:logHelper.stringify(eventEssentials(event))});
			return error;
		};
		log.debug({operation:'processEvent', result:result, event:logHelper.stringify(eventEssentials(event))});
		resolve(result);
	})
	.then(result=>{
...
function eventEssentials(event) {
	return {resource: event.resource, path:event.path, resourcePath:event.resourcePath, requestContext: event.requestContext, headers: event.headers, body:event.body};
}
